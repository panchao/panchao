...
;(function main($) {
  ...
  // 获取用户列表
  $.get('../query/getPhotographerUsers?photographers_id='+ photographerId, customerList.update('customers'));   //TODO
  ...
  $next.on('click', function () {
    ...
    if ($currentStep.data('step') === 'new-order') {
      ...
      if (!allFilled) {
        ...
        $.ajax({
          type: "POST",
          url: "../order/addOrder.do",    //TODO
          data: $orderForm.serialize(),
          success: function(msg){
              msg.retCode == '200' && (orderId = msg.orderId);
          }
        });
    ...
    if ($nextStep.data('step') === 'photo-upload') { // 当前是第三步，上传图片
      ...
      $.getJSON('../pictures/photographersAlbums.do?photograper_id='+photographerId, updateCustomerList);   //TODO
    }
    else {
      $entry.addClass('hide');
    }
    ...
    if ($nextStep[0].id === 'entry-code') {
      $.get('/order/generateEntrance.do?orderId=' + orderId, function (msg) {    //TODO
        if (msg.retCode == '200') {
  ...
  $photoBox.on('click', '.album', function () {
    ...
    $.getJSON('/pictures/album.do?photograper_id='+photographerId+'&album_id='+ $(this).data('album').id, updateCustomerList);  //TODO
  });
  ...
  $('#createCustomerForm').formValidation({
    ...
    $('#createCustomerForm').submit(function () {
      if ($('.has-error').length === 0) {
        ...
        $.ajax({
          type: "POST",
          url: "../customer/addCustomer.do",    //TODO
